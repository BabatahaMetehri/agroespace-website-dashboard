...
-- IMPORTANT: These must be created BEFORE the policies that use them.
-- SECURITY DEFINER means these functions run with superuser-level access,
-- bypassing RLS on the profiles table — which prevents infinite recursion.
...
-- ─── 4. Profiles policies ─────────────────────────────────────────────────
-- Drop any existing policies first to allow clean re-run
DROP POLICY IF EXISTS "profiles_self_read" ON profiles;
DROP POLICY IF EXISTS "profiles_admin_read_all" ON profiles;
DROP POLICY IF EXISTS "profiles_admin_write" ON profiles;

-- Every user can read their own profile row (needed so the app can fetch role)
CREATE POLICY "profiles_self_read" ON profiles
  FOR SELECT USING (auth.uid() = id);

-- Admins can read ALL profiles (e.g. to list users).
-- We use public.is_admin() (SECURITY DEFINER) instead of an inline subquery
-- on `profiles`, because an inline EXISTS (SELECT … FROM profiles …) inside a
-- policy on `profiles` causes infinite recursion in PostgreSQL's RLS engine.
...
-- ─── 5. Drop the old wide-open policies ───────────────────────────────────
...
-- Also drop new policies if re-running migration (idempotent)
DROP POLICY IF EXISTS "inst_read_authed" ON installations;
DROP POLICY IF EXISTS "inst_insert_admin" ON installations;
DROP POLICY IF EXISTS "inst_update_admin" ON installations;
DROP POLICY IF EXISTS "inst_delete_admin" ON installations;
DROP POLICY IF EXISTS "logs_read_authed" ON maintenance_logs;
DROP POLICY IF EXISTS "logs_insert_engadmin" ON maintenance_logs;
DROP POLICY IF EXISTS "logs_update_engadmin" ON maintenance_logs;
DROP POLICY IF EXISTS "logs_delete_admin" ON maintenance_logs;
DROP POLICY IF EXISTS "photos_read_authed" ON installation_photos;
DROP POLICY IF EXISTS "photos_insert_engadmin" ON installation_photos;
DROP POLICY IF EXISTS "photos_update_engadmin" ON installation_photos;
DROP POLICY IF EXISTS "photos_delete_admin" ON installation_photos;

-- ─── 6. Installations: read = anyone authed, write = admin only ──────────
...
-- ─── 7. Maintenance logs: read authed, write engineer+admin ──────────────
...
-- ─── 8. Installation photos: read authed, write engineer+admin ───────────
...
-- ─── 9. Promote your account to admin (RUN ONCE, EDIT EMAIL FIRST) ───────
...
--   2. Run:  UPDATE public.profiles SET role = 'engineer' WHERE email = 'them@example.com';
-- To promote to admin:  UPDATE public.profiles SET role = 'admin' WHERE email = '…';
-- To revoke (back to viewer):  UPDATE public.profiles SET role = 'viewer' WHERE email = '…';
